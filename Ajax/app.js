...
// ========================================
// SETUP AJAX WITH TOKEN HEADER
// ========================================
function setupAjaxTokenHeader() {
    // Add authorization token to all AJAX requests
    $.ajaxSetup({
...
function showMainApp(email) {
    ...
    // Setup AJAX with token header for all subsequent requests
...
function renderCurrentCategory() {
    ...
    // Render combined mapping table (source + mapping rows aligned horizontally)
    renderMappingTable();
    
    // Render destination tabs first (so they're visible)
    updateDestinationCategoryDisplay();
    
    // Render destination accounts
    renderDestinationAccounts();
    
    // Setup drag and drop for the new row-based layout
...
function setupSynchronizedScrolling() {
    ...
    // If any previous sync listeners exist, remove them first.
